...
dreamspon_login_url="https://www.dreamspon.com/member/login.html"
...
# url을 이용하여 브라우저로 접속
driver.get(dreamspon_login_url)
driver.implicitly_wait(3)
driver.find_element(By.NAME, 'mbr_id').send_keys('gustmd5485@naver.com')
driver.find_element(By.NAME, 'pwd_in').send_keys('rlagustmd')
#driver.find_element(By.NAME, 'login_id_save').click()
time.sleep(1)
driver.find_element(By.XPATH, '//*[@id="loginForm"]/div[1]/input').click()

driver.implicitly_wait(3)
driver.get("https://www.dreamspon.com/scholarship/view.html?idx=6406")
html = driver.page_source
print(html)
soup = BeautifulSoup(html, 'html.parser')
result = soup.find_all('dl', class_="scholarship04")
print(result)
"""
#driver.quit()
...
aws_region = os.getenv('REGION_KEY')  # 예: 'us-east-1'

# SQS 큐 URL 설정
sqs_queue_url = 'https://sqs.ap-northeast-2.amazonaws.com/855774094228/skkuolarship-dev-extract'

# 크롤링한 데이터를 SQS에 보내는 함수
def send_data_to_sqs(data):
    ...
    # 응답 출력 (성공 여부 확인)
    print("Message sent to SQS. MessageId:", response['MessageId'])
...
for department_name, department_url, department_base_url in departments:
    ...
    if crawled_data_list:
        for data in crawled_data_list:
            send_data_to_sqs(json.dumps(data))
            print(f"Data for {department_name} sent to SQS.")
    else:
